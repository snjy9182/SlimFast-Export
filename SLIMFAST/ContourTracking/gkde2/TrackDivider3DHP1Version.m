...
figure;
for ii = 1:9
subplot(3,3,ii), imagesc(mask(:,:,ii));
end
...
pcutoffIn = 0.2;
pcutoffOut = 0.2;
MaskIn = (mask > pcutoffIn);
MaskOut = (mask < pcutoffOut);

figure;
for ii = 1:9
subplot(3,3,ii), imshow(MaskIn(:,:,ii));
end

figure;
for ii = 1:9
subplot(3,3,ii), imshow(MaskOut(:,:,ii));
end


%% Track Divider
[FileName2,PathName2] = uigetfile('*.mat', 'Select the u-track mat file for Diffusion Analysis (10ms)');
fh2 = fullfile(PathName2, FileName2);
load(fh2);
Tracks = [];
for i = 1:length(tracks)
    trk = tracks{i};
    trks = [trk(:,1)/(Pixelxy*1000) trk(:,2)/(Pixelxy*1000) trk(:,3)/(Pixelz*1000) trk(:,5) ones(size(trk, 1), 1)*i];
    Tracks = vertcat(Tracks, trks);
end
...
hp = max(mask, [], 3);
xImg = linspace(0, size(hp, 2), size(hp, 2));
yImg = linspace(0, size(hp, 1), size(hp, 1));
figure;
image(xImg, yImg, hp, 'CDataMapping', 'scaled');
hold on;
plot(Inmask(:,1), Inmask(:,2), 'xg');
xlim([0, size(hp, 2)]);
ylim([0, size(hp, 1)]);
title('Overlapped');
xlabel('X(pixel)');
...
xz = squeeze(max(MaskIn(:,:, 1:9), [], 2));
xImg = linspace(0, size(xz, 1), size(xz, 1));
zImg = linspace(0, size(xz, 2), size(xz, 2));
figure;
image(xImg, zImg, xz, 'CDataMapping', 'scaled');
hold on;
plot(Outmask(:,1), Outmask(:,3), 'xg');
xlim([0, size(xz, 1)]);
ylim([0, size(xz, 2)]);
title('Overlapped');
xlabel('X(pixel)');
